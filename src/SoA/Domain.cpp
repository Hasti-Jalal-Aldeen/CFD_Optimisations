...
{
    C[0] = (int*) _mm_malloc(sizeof(int)*Nf, ALIGN);
    C[1] = (int*) _mm_malloc(sizeof(int)*Nf, ALIGN);
    for (int dim = 0; dim < NDIMS; dim++) {
        rf[dim][0] = (double*) _mm_malloc(sizeof(double)*Nf, ALIGN); setarr(0, Nf, 0.0, rf[dim][0]);
        rf[dim][1] = (double*) _mm_malloc(sizeof(double)*Nf, ALIGN); setarr(0, Nf, 0.0, rf[dim][1]);
        xf[dim]    = (double*) _mm_malloc(sizeof(double)*Nf, ALIGN); setarr(0, Nf, 0.0, xf[dim]);
        nf[dim]    = (double*) _mm_malloc(sizeof(double)*Nf, ALIGN); setarr(0, Nf, 0.0, nf[dim]);
        xc[dim]    = (double*) _mm_malloc(sizeof(double)*Nc, ALIGN); setarr(0, Nc, 0.0, xc[dim]);
    }
    Af = (double*) _mm_malloc(sizeof(double)*Nf, ALIGN); setarr(0, Nf, 0.0, Af);
    If = (double*) _mm_malloc(sizeof(double)*Nf, ALIGN); setarr(0, Nf, 0.0, If);
...
void Domain::compute_iv() {
    for (int f = 0; f < Nf; f++) {
        ...
        int CL = C[f][0];
        int CR = C[f][1];
        iv[CL] += A*nf_dot_rL/NDIMS;
        iv[CR] -= A*nf_dot_rR/NDIMS;
    }
