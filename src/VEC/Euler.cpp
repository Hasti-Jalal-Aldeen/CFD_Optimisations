...
: nvars(nvars), Nc(domain.Nc), Nf(domain.Nf), NBC(domain.NBC), domain(domain), flux(flux), grads(grads), limiter(limiter)
{
    ...
    for (int var = 0; var < nvars; var++) {
        Q[var]    = (double*) _mm_malloc(sizeof(double)*(Nc+1), ALIGN); setarr(0, Nc+1, 0.0, Q[var]);
        U[var]    = (double*) _mm_malloc(sizeof(double)*(Nc+1), ALIGN); setarr(0, Nc+1, 0.0, U[var]);
        R[var]    = (double*) _mm_malloc(sizeof(double)*(Nc+1), ALIGN); setarr(0, Nc+1, 0.0, R[var]);
        phi[var]  = (double*) _mm_malloc(sizeof(double)*(Nc+1), ALIGN); setarr(0, Nc+1, 0.0, phi[var]);
        G[var]    = (double*) _mm_malloc(sizeof(double)*(NDIMS*Nc+NDIMS), ALIGN); setarr(0, NDIMS*Nc+NDIMS, 0.0, G[var]);
    }
    ...
    #endif
    writer = new Writer(nvars, "../sols/VEC", varnames, Nc, Q);
